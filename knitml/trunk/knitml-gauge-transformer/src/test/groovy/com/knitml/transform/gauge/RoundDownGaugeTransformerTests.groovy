...
class RoundDownGaugeTransformerTests extends AbstractTransformerTests {
	
	void setUpGauges(TransformHandler handler) {
		handler.originalGauge = Measure.valueOf(5, STITCHES_PER_INCH)
		handler.targetGauge = Measure.valueOf(4 + 1/3, STITCHES_PER_INCH)
	}
	
	void validate (Row row) {
		assertThat row.information.details[0].label, is ('with 3 fewer st')
	}

	void validate (CastOn castOn) {
		assertThat castOn.numberOfStitches, is (20)
		assertThat castOn.annotation, containsString (17.toString())
	}
	
	void validateChange (Row row, int delta) {
		if (delta != 0) {
			assertThat row.information.details[1], not (null)
			int total = round ( (20 + delta) * ( (4+1/3)/5 ))
